...
export async function createDraftSession(config: {
  ...
  try {
    ...
    const { data: draftOrder } = await supabase.from("draft_order").select("id").eq("season_id", activeSeason.id).limit(1);
    if (!draftOrder || draftOrder.length === 0) return { success: false, error: "No draft order found for the active season." };
...
export async function getActiveDraftSession(): Promise<{ session: DraftSessionWithStatus | null; error?: string; }> {
  ...
  } catch (error) { return { session: null, error: String(error) }; }
}

// THIS IS THE FIX: Export getDraftSessions which returns the full object
...
export async function deleteDraftSession(sessionId: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        if (session.status !== "scheduled" && session.status !== "completed") return { success: false, error: "Can only delete scheduled or completed draft sessions." };
...
async function completeDraftInternal(sessionId: string, adminClient?: AnySupabaseClient): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        if (error) return { success: false, error: error.message };
        await supabase.rpc("notify_all_users_draft", { p_notification_type: "draft_completed", p_message: "The draft has been automatically completed." });
        return { success: true };
    } catch (error) { return { success: false, error: String(error) }; }
}
...
export async function checkDraftTimer(adminClient?: AnySupabaseClient): Promise<{
  ...
  error?: string;
    ...
    try {
        ...
        const advanceResult = await advanceDraft(sessionId, adminClient);
        if (!advanceResult.success) return { action: "error", error: `Skip logged but failed to advance draft: ${advanceResult.error}` };
