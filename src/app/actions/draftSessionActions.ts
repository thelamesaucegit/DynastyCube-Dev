...
export interface DraftSession {
  id: string; name: string; season_id: string; status: "scheduled" | "active" | "paused" | "completed";
  total_rounds: number; hours_per_pick: number; start_time: string; end_time: string | null;
  autodraft_next_pick_at: string | null; current_on_clock_team_id: string | null; started_by: string | null;
  created_at: string; updated_at: string; consecutive_skipped_picks?: number; current_pick_deadline: string | null;
}
export interface DraftSessionWithStatus extends DraftSession { draftStatus: DraftStatus | null; }
export interface DraftSessionInfo { id: string; name: string; created_at: string; }
...
export async function resetSkipCounter(sessionId: string, adminClient?: AnySupabaseClient) {
...
export async function createDraftSession(config: { totalRounds: number; hoursPerPick: number; startTime: string; endTime?: string; }): Promise<{ success: boolean; sessionId?: string; error?: string; }> {
    try {
        ...
        const { data, error } = await supabase.from("draft_sessions").insert({ season_id: activeSeason.id, name: newDraftName, status: "scheduled", total_rounds: config.totalRounds, hours_per_pick: config.hoursPerPick, start_time: config.startTime, end_time: config.endTime || null, started_by: admin.userId }).select().single();
...
export async function getActiveDraftSession(): Promise<{ session: DraftSessionWithStatus | null; error?: string; }> {
    ...
    } catch (e) { return { session: null, error: String(e) }; }
}
...
async function completeDraftInternal(sessionId: string, adminClient?: AnySupabaseClient): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        if (error) return { success: false, error: error.message };
        await supabase.rpc("notify_all_users_draft", { p_notification_type: "draft_completed", p_message: "The draft has completed." });
        return { success: true };
    } catch (e) { return { success: false, error: String(e) }; }
}
...
export async function activateDraft(sessionId: string, adminClient?: AnySupabaseClient): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        if (session.status !== "scheduled" && session.status !== "paused") return { success: false, error: `Cannot activate a draft with status: ${session.status}` };
        ...
        const { error: updateError } = await supabase.from("draft_sessions").update({ status: "active", autodraft_next_pick_at: deadline.toISOString(), current_on_clock_team_id: draftStatus.onTheClock.teamId, updated_at: now.toISOString() }).eq("id", sessionId);
        if (updateError) return { success: false, error: updateError.message };
        await supabase.rpc("notify_all_users_draft", { p_notification_type: "draft_started", p_message: `The draft has started!` });
        await supabase.rpc("notify_draft_team_roles", { p_team_id: draftStatus.onTheClock.teamId, p_notification_type: "draft_on_clock", p_message: `You are on the clock! You have ${session.hours_per_pick} hours to pick.` });
        if (draftStatus.onDeck.teamId !== draftStatus.onTheClock.teamId) await supabase.rpc("notify_draft_team_roles", { p_team_id: draftStatus.onDeck.teamId, p_notification_type: "draft_on_deck", p_message: `You are on deck!` });
        return { success: true };
    } catch(e) { return { success: false, error: String(e) }; }
}
...
export async function advanceDraft(sessionId: string, adminClient?: AnySupabaseClient): Promise<{ success: boolean; completed?: boolean; error?: string }> {
    try {
        ...
        if (allTeamsReachedRounds) {
          await completeDraftInternal(session.id, adminClient);
          return { success: true, completed: true };
        }
        const now = new Date();
        const deadline = new Date(now.getTime() + Number(session.hours_per_pick) * 60 * 60 * 1000);
        await supabase.from("draft_sessions").update({ autodraft_next_pick_at: deadline.toISOString(), current_on_clock_team_id: draftStatus.onTheClock.teamId, updated_at: now.toISOString() }).eq("id", session.id);
        await supabase.rpc("notify_draft_team_roles", { p_team_id: draftStatus.onTheClock.teamId, p_notification_type: "draft_on_clock", p_message: `You are on the clock! Round ${draftStatus.currentRound}.` });
        if (draftStatus.onDeck.teamId !== draftStatus.onTheClock.teamId) await supabase.rpc("notify_draft_team_roles", { p_team_id: draftStatus.onDeck.teamId, p_notification_type: "draft_on_deck", p_message: `You are on deck!` });
...
export async function pauseDraft(sessionId: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        const admin = await verifyAdmin(supabase);
        if (!admin.authorized) return { success: false, error: admin.error };
        const { error } = await supabase.from("draft_sessions").update({ status: "paused", autodraft_next_pick_at: null, updated_at: new Date().toISOString() }).eq("id", sessionId).eq("status", "active");
        if (error) return { success: false, error: error.message };
        await supabase.rpc("notify_all_users_draft", { p_notification_type: "draft_paused", p_message: "The draft has been paused." });
        return { success: true };
    } catch(e) { return { success: false, error: String(e) }; }
}
...
export async function checkDraftTimer(): Promise<{ action: "none" | "activated" | "auto_drafted" | "completed" | "error"; message?: string; error?: string; }> {
  ...
  try {
    ...
    const { data: expiredSession } = await supabase.from("draft_sessions").update({ autodraft_next_pick_at: null, updated_at: now.toISOString() }).eq("status", "active").not("autodraft_next_pick_at", "is", null).lte("autodraft_next_pick_at", now.toISOString()).select().maybeSingle();
