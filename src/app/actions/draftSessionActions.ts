...
async function getTeamMemberCount(teamId: string, client?: AnySupabaseClient): Promise<number> {
...
async function executeConfirmedTeamPick(
  ...
  const { data, error } = await supabase.rpc("execute_atomic_draft_pick", {
    ...
    p_pick_number: existingPicks.length + 1, p_cost: card.cubucks_cost || 1,
...
): Promise<{ card: CardData | null; algorithmDetails: AlgorithmDetails | null; error?: string; }> {
  try {
    ...
    const { picks: teamPicks } = await getTeamDraftPicks(teamId, draftSessionId, client);
    const { team: teamBalance } = await getTeamBalance(teamId, client);
    ...
    if (selectedCard && (selectedCard.cubucks_cost || 1) > balance) {
        ...
        else { const anyAffordable = availableCards.filter(c => (c.cubucks_cost || 1) <= balance).sort((a, b) => (b.cubecobra_elo || 0) - (a.cubecobra_elo || 0)); selectedCard = anyAffordable[0] || null; }
    }
        ...
        if (queueEntries && queueEntries.length > 0) {
            ...
            for (const entry of queueEntries) {
                if (entry.card_pool_id && availableInstanceIds.has(entry.card_pool_id)) {
                    ...
                    const memberCount = await getTeamMemberCount(teamId, supabase); // Pass client
                    return { nextPick: card, source: "manual_queue", queueDepth, votes: entry.votes || [], voteThreshold: calculateVoteThreshold(memberCount), };
                }
            }
        }
        const { card, algorithmDetails, error } = await computeAutoDraftPick(teamId, draftSessionId, supabase);
        if (card) {
...
export async function setTeamDraftQueue(teamId: string, entries: Array<{ cardPoolId: string; cardId: string; cardName: string; position: number; pinned?: boolean; }>): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        return { success: true };
    } catch (error) { return { success: false, error: "Failed to update draft queue" }; }
}
...
export async function pinCardToQueue(teamId: string, cardPoolId: string, cardId: string, cardName: string, position: number = 1): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        if (insertError) return { success: false, error: insertError.message };
        return { success: true };
    } catch (error) { return { success: false, error: "Failed to pin card to queue" }; }
}
...
export async function removeFromQueue(teamId: string, cardPoolId: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        return { success: true };
    } catch (error) { return { success: false, error: "Failed to remove from queue" }; }
}
...
export async function clearTeamDraftQueue(teamId: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        return { success: true };
    } catch (error) { return { success: false, error: "Failed to clear draft queue" }; }
}
...
export async function executeAutoDraft(
  ...
  try {
    ...
    supabase.channel(`draft-updates-${draftSessionId}`).send({ type: 'broadcast', event: 'new_pick', payload: broadcastPayload });
