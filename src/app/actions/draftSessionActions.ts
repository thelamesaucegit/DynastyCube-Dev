...
export async function createDraftSession(config: {
  ...
  try {
    ...
    const { data: activeSeason } = await supabase
      .from("seasons")
      .select("id, season_name") // Also get season_name
      .eq("is_active", true)
      .single();
    ...
    // Count existing drafts for this season to generate the name
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        name: newDraftName, // Set the name on creation
        status: "scheduled",
...
export async function updateDraftSession(
  ...
  updates: {
    ...
    name?: string; // Allow name to be updated
  }
...
export async function deleteDraftSession(
  ...
  try {
    ...
    // Allow deletion for completed drafts too
