...
export interface DraftSession {
  ...
  current_pick_deadline: string | null; // This is now deprecated in favor of autodraft_next_pick_at
}
...
export async function checkDraftTimer(): Promise<{ action: "none" | "activated" | "auto_drafted" | "completed" | "error"; message?: string; error?: string; }> {
  ...
  try {
    ...
    if (!teamId) return { action: "error", error: "Expired session has no on-the-clock team." };
