...
// THE FIX: Added season_id string property explicitly
...
export async function updateDraftSetting(
  ...
  try {
    ...
    const { error } = await supabase
      .from("draft_settings")
      .upsert(
        { setting_key: key, setting_value: value, updated_by: admin.userId },
  ...
  try {
    ...
    if (weekIds.length === 0) {
      const standings: SeasonStanding[] = teams.map((team: { id: string; name: string; emoji: string }) => ({
        team_id: team.id, team_name: team.name, emoji: team.emoji, wins: 0, losses: 0, win_pct: 0,
      }));
      return { standings };
    }
    ...
    const standings: SeasonStanding[] = teams.map((team: { id: string; name: string; emoji: string }) => {
      ...
      return {
        team_id: team.id, team_name: team.name, emoji: team.emoji,
        wins: stats.wins, losses: stats.losses, win_pct: Math.round(winPct * 100) / 100,
      };
    });
  ...
  try {
    ...
    const resolvePickOwner = (globalPickNum: number) => {
        ...
        if (tradeMap.has(tradeKey)) {
            ...
            // Lookup the new owner's details, falling back to original if somehow missing from active roster
            const newOwner = draftOrderTeams.find(t => t.teamId === newOwnerId);
            
            if (newOwner) return newOwner;

            // THE FIX: If the traded team is a Hidden Team (like Drainlings), they won't be in draftOrderTeams!
            // We return a mock placeholder for them to trigger special logic correctly!
            if (newOwnerId === "90177632-f6ab-4501-b235-3590a7e46472") {
                return {
                    teamId: newOwnerId,
                    teamName: "Drainlings of the Blind Eternities",
                    teamEmoji: "⚫",
                    pickPosition: originalOwner.pickPosition
                };
            }
        }
    ...
    // THE FIX: Added season_id to the output!
    return {
      status: { onTheClock, onDeck, currentRound, totalPicks, totalTeams, seasonName: activeSeason.season_name, season_id: activeSeason.id, draftOrder },
      seasonId: activeSeason.id,
    };
  } catch (error) {
