...
// 1. Custom Event to trigger the modal from anywhere
...
export const ReportButton: React.FC<{ onClick?: () => void }> = ({ onClick }) => {
  const handleClick = () => {
    openReportModal();
    if (onClick) onClick(); // Allows us to close the mobile menu!
  };
...
// 3. The Global Modal Component
export const ReportModalOverlay: React.FC = () => {
      ...
      <div className="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                >
                  <div className="font-bold text-sm text-gray-900 dark:text-gray-100">{type.label}</div>
          ...
          {reportType === "bad_actor" && (
            <div>
              <label className="block text-xs uppercase tracking-wider font-bold text-gray-500 dark:text-gray-400 mb-1.5">Select User</label>
              <select
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider font-bold text-gray-500 dark:text-gray-400 mb-1.5">Description</label>
            <textarea
              value={description}
