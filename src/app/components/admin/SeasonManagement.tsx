        ...
        <>
          ...
          {showRolloverDetails && rolloverChanges.length > 0 && (
            <div className="mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                <div className="p-4 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center"><h4 className="font-semibold text-gray-900 dark:text-gray-100">Rollover Changes ({rolloverChanges.length} cards)</h4><button onClick={() => setShowRolloverDetails(false)} className="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100">Hide</button></div>
                <div className="max-h-96 overflow-y-auto"><table className="w-full text-sm">
                    <thead className="bg-gray-50 dark:bg-gray-900 sticky top-0"><tr>
                        <th className="px-4 py-2 text-left font-semibold">Card</th><th className="px-4 py-2 text-center font-semibold">Old Cost</th><th className="px-4 py-2 text-center font-semibold">New Cost</th>
                        <th className="px-4 py-2 text-center font-semibold">Change</th><th className="px-4 py-2 text-center font-semibold">Drafted?</th>
                    </tr></thead>
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {rolloverChanges.map((change, idx) => {
                            const diff = change.new_cost - change.old_cost;
                            return (<tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
            ...
            <div className="space-y-3">
              {seasons.map((season) => (
                <div key={season.id} className={`bg-white dark:bg-gray-800 border rounded-lg p-4 ${season.is_active ? "border-blue-500 dark:border-blue-400 shadow-md" : "border-gray-200 dark:border-gray-700"}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-3"><h4 className="font-semibold text-gray-900 dark:text-gray-100">{season.season_name}</h4>{season.is_active && (<span className="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded text-xs font-medium">Active</span>)}</div>
