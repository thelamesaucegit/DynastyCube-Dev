...
export const DraftSessionManagement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      const deadline = new Date(activeSession.autodraft_next_pick_at!).getTime();
      const now = Date.now();
  ...
  const handleToggleTestMode = async (sessionId: string, currentHours: number) => {
    ...
    if (!confirm(`Switch pick timer to ${label}? This will reset the current pick's timer.`)) return;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "active": return <span className="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-full text-sm font-medium animate-pulse">Live</span>;
      case "paused": return <span className="px-3 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 rounded-full text-sm font-medium">Paused</span>;
      case "completed": return <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full text-sm font-medium">Completed</span>;
      default: return null;
    }
  };
  ...
  return (
    <div className="admin-section">
      ...
      {activeSession && (
        <div className={`mb-6 p-5 rounded-lg border-2 ${activeSession.status === "active" ? "border-green-400 dark:border-green-600 bg-green-50/50 dark:bg-green-900/10" : activeSession.status === "paused" ? "border-yellow-400 dark:border-yellow-600 bg-yellow-50/50 dark:bg-yellow-900/10" : "border-blue-400 dark:border-blue-600 bg-blue-50/50 dark:bg-blue-900/10"}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Current Draft Session</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div>
              ...
              <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatDateTime(activeSession.start_time)}</div>
            </div>
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">End</div>
              <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{activeSession.end_time ? formatDateTime(activeSession.end_time) : "No deadline"}</div>
            </div>
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Rounds</div>
              <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{activeSession.draftStatus ? `Round ${activeSession.draftStatus.currentRound} of ${activeSession.total_rounds}`: activeSession.total_rounds}</div>
            </div>
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Hours/Pick</div>
              {editingHours ? (
                <div className="flex gap-1 items-center">
                  <input type="number" step="0.5" value={editHoursValue} onChange={(e) => setEditHoursValue(e.target.value)} className="w-16 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-sm" autoFocus />
                  <button onClick={() => handleUpdateHours(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-primary text-xs py-1 px-2">Save</button>
                  <button onClick={() => setEditingHours(false)} className="admin-btn admin-btn-secondary text-xs py-1 px-2">✕</button>
                </div>
              ) : (
                <div className={`text-sm font-medium cursor-pointer hover:text-blue-600 ${activeSession.hours_per_pick <= TEST_MODE_HOURS + 0.001 ? "text-orange-600 dark:text-orange-400" : "text-gray-900 dark:text-gray-100"}`} onClick={() => { setEditHoursValue(String(activeSession.hours_per_pick)); setEditingHours(true); }} title="Click to edit">
                  {activeSession.hours_per_pick <= TEST_MODE_HOURS + 0.001 ? "1m (TEST)" : `${activeSession.hours_per_pick}h`}
                </div>
              )}
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="font-bold text-amber-900 dark:text-amber-100 text-lg">{activeSession.draftStatus.onTheClock.teamName}</span>
                    <span className="text-amber-700 dark:text-amber-300 text-sm">is on the clock</span>
                  </div>
                  {activeSession.draftStatus.onDeck && (<div className="text-sm text-amber-700 dark:text-amber-300">On Deck: {activeSession.draftStatus.onDeck.teamEmoji} {activeSession.draftStatus.onDeck.teamName}</div>)}
                </div>
                <div className="text-right">
                  <div className="text-xs text-amber-600 dark:text-amber-400 uppercase tracking-wide mb-1">Auto-draft in</div>
                  <div className="text-2xl font-mono font-bold text-amber-900 dark:text-amber-100">{countdown || "..."}</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            {activeSession.status === "scheduled" && (
              <>
                <button onClick={() => handleActivate(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-primary">{actionLoading ? "Starting..." : "Start Draft Now"}</button>
                <button onClick={() => handleDelete(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-danger">Delete</button>
              </>
            )}
            {activeSession.status === "active" && (
              <>
                <button onClick={() => handlePause(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-secondary">{actionLoading ? "Pausing..." : "Pause Draft"}</button>
                <button onClick={() => handleToggleTestMode(activeSession.id, activeSession.hours_per_pick)} disabled={testModeLoading} title="Sets pick timer to 1 minute for rapid auto-draft testing" className={`admin-btn ${activeSession.hours_per_pick <= TEST_MODE_HOURS + 0.001 ? "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border border-orange-300 dark:border-orange-600 hover:bg-orange-200 dark:hover:bg-orange-900/50" : "admin-btn-secondary"}`}>
                  {testModeLoading ? "Updating..." : activeSession.hours_per_pick <= TEST_MODE_HOURS + 0.001 ? "Disable Test Mode" : "Test Mode (1 min)"}
                </button>
                <button onClick={() => handleComplete(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-danger">{actionLoading ? "Ending..." : "End Draft Early"}</button>
              </>
            )}
            {activeSession.status === "paused" && (
              <>
                <button onClick={() => handleResume(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-primary">{actionLoading ? "Resuming..." : "Resume Draft"}</button>
                <button onClick={() => handleComplete(activeSession.id)} disabled={actionLoading} className="admin-btn admin-btn-danger">{actionLoading ? "Ending..." : "End Draft Early"}</button>
              </>
            )}
          </div>
        ...
        <div className="mb-6">
          {showCreateForm ? (
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Schedule New Draft</h3>
              {draftOrder.length > 0 ? (
                <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {draftOrder.map((entry) => {
                      ...
                      return (<span key={entry.id} className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded text-sm"><span className="font-bold text-gray-500 dark:text-gray-400 text-xs">{entry.pick_position}.</span><span>{team?.emoji}</span><span className="text-gray-900 dark:text-gray-100">{team?.name}</span></span>);
                    })}
                  </div>
                </div>
              ) : (
                <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 rounded-lg">
                  <p className="text-sm text-red-700 dark:text-red-300">No draft order found. Please generate a draft order in the Draft Order tab first.</p>
                </div>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Start Date *</label>
                  <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Start Time *</label>
                  <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Date <span className="text-gray-400">(optional)</span></label>
                  <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Time <span className="text-gray-400">(optional)</span></label>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setShowCreateForm(false); resetForm(); }} className="admin-btn admin-btn-secondary">Cancel</button>
              </div>
            </div>
          ) : (
            <button onClick={() => setShowCreateForm(true)} className="admin-btn admin-btn-primary">+ Schedule New Draft</button>
          )}
        </div>
      )}
      {allSessions.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Draft Session History</h3>
