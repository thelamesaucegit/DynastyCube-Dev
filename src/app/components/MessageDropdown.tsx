...
export const MessageDropdown: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        document.getElementById("message-portal-container")?.contains(target)
      ) {
        return;
      }
      setIsOpen(false);
  ...
  // THE FIX: Calculate exact viewport positions dynamically
  const getDropdownStyles = (): React.CSSProperties => {
    ...
    if (isMobile) {
      // Mobile Layout: Center the panel over the viewport under the header
      return {
        position: "fixed",
        top: "70px",
        left: "4%",
    ...
    // Desktop Layout: Align right-aligned directly below the trigger button
    return {
      position: "fixed",
      top: `${rect.bottom + window.scrollY + 10}px`,
      left: `${rect.right - 384}px`, // 384px matches 'w-96'
      width: "384px",
      zIndex: 9999,
    };
  };
  ...
  const dropdownContent = isOpen ? (
    <div
      id="message-portal-container"
      ref={dropdownRef}
      style={getDropdownStyles()}
      className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[420px] animate-in fade-in slide-in-from-top-2 duration-150"
    >
      {/* Header */}
  ...
  return (
    <div className="relative">
      ...
      {/* THE PORTAL: Escape the sticky navigation header bounds securely on mobile and desktop! [1] */}
