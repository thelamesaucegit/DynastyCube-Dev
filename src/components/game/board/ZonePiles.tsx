...
export function ZonePile({ player, isOpponent = false }: ZonePileProps) {
  ...
  }, [player.librarySize, liveGraveyardCards, liveExileCards, liveStackCards]);
  ...
  return (
    <>
      <div style={{ ...styles.zonePile, gap: responsive.cardGap, minWidth: responsive.pileWidth + 10, ...verticalOffset }}>
        ...
        <div style={styles.zoneStack}>
          <div data-graveyard-id={player.playerId} style={{ ...styles.graveyardPile, ...pileStyle, cursor: graveyardCards.length > 0 ? 'pointer' : 'default' }} onClick={() => { if (graveyardCards.length > 0) setBrowsingGraveyard(true) }} onMouseEnter={() => { if (topGraveyardCard) hoverCard(topGraveyardCard.id) }} onMouseLeave={() => hoverCard(null)}>
            {topGraveyardCard ? <img src={getCardImageUrl(topGraveyardCard.name, topGraveyardCard.imageUri, 'normal')} alt={topGraveyardCard.name} style={{ ...styles.pileImage, opacity: 0.8 }} onError={(e) => handleImageError(e, topGraveyardCard.name, 'normal')} /> : <div style={styles.emptyPile} />}
            ...
            {targetedGraveyardCards.map((card, index) => {
              ...
              return (
                <div key={card.id} data-card-id={card.id} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 10 + index, boxShadow: '0 0 12px 4px rgba(255, 136, 0, 0.8)', borderRadius: responsive.isMobile ? 4 : 6, transform: `translateX(${fanOffset}px)` }}>
                  <img src={getCardImageUrl(card.name, card.imageUri, 'normal')} alt={card.name} style={{ ...styles.pileImage, borderRadius: responsive.isMobile ? 4 : 6 }} onError={(e) => handleImageError(e, card.name, 'normal')} />
                </div>
              );
            })}
          </div>
        ...
        <div style={styles.zoneStack}>
          <div data-exile-id={player.playerId} style={{ ...styles.exilePile, ...pileStyle, cursor: exileCards.length > 0 ? 'pointer' : 'default' }} onClick={() => { if (exileCards.length > 0) setBrowsingExile(true) }} onMouseEnter={() => { if (topExileCard) hoverCard(topExileCard.id) }} onMouseLeave={() => hoverCard(null)}>
            {topExileCard ? <img src={getCardImageUrl(topExileCard.name, topExileCard.imageUri, 'normal')} alt={topExileCard.name} style={{ ...styles.pileImage, opacity: 0.7 }} onError={(e) => handleImageError(e, topExileCard.name, 'normal')} /> : <div style={styles.emptyPile} />}
      ...
      {browsingGraveyard && <ZoneBrowser zoneName="Graveyard" cards={graveyardCards} onClose={() => setBrowsingGraveyard(false)} />}
      {browsingExile && <ZoneBrowser zoneName="Exile" cards={exileCards} onClose={() => setBrowsingExile(false)} />}
    </>
  );
}

function ZoneBrowser({ zoneName, cards, onClose }: ZoneBrowserProps) {
  const hoverCard = useGameStore((state) => state.hoverCard);
  const responsive = useResponsiveContext();
  ...
  // Use the style from the imported styles object
  const buttonStyle = { ...styles.restoreButtonStyle, backgroundColor: zoneName === 'Graveyard' ? '#1e40af' : '#7c3aed' };

  if (minimized) {
    return <button onClick={() => setMinimized(false)} style={buttonStyle}>↑ Return to {zoneName}</button>;
  }
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 16, justifyContent: 'center' }}>
          <button onClick={() => setMinimized(true)} style={{...buttonStyle, position: 'relative', bottom: 'auto', left: 'auto', transform: 'none' }}>View Battlefield</button>
